...
ipcRenderer.on('tiny-notification-create-confirm', (_event, arg) => {
  if (
    notifications[arg.tag] &&
    !notifications[arg.tag].validated &&
    typeof notifications[arg.tag].resolve === 'function'
  ) {
...
ipcRenderer.on('tiny-notification-failed', (_event, arg) => {
  if (notifications[arg.tag]?.event)
    notifications[arg.tag].event.emit(
      'failed',
      new Error(
        arg.error?.message ? arg.error.message : arg.error,
        arg.error?.fileName,
