...
const createTinyCache = (event, data, callback, priorityItem = 0) => {
  if (typeof event === 'string' && objType(data, 'object') && typeof callback === 'function') {
    ...
    if (typeof priority !== 'number' || Number.isNaN(priority) || !Number.isFinite(priority))
      priority = 0;
...
tinyAPI.on = (event, callback, priority = 0) =>
  createTinyCache(event, { type: 'on' }, callback, priority);

tinyAPI.once = (event, callback, priority = 0) =>
  createTinyCache(event, { type: 'once' }, callback, priority);
  ...
  if (Array.isArray(tinyPlugins.order[event])) {
    for (const item in tinyPlugins.order[event]) {
      ...
      if (tinyPlugins.order[event][item].type === 'once') {
        deleteTinyCache(
          event,
          tinyPlugins.order[event][item].callback,
          tinyPlugins.order[event][item].index,
        );
      }
    }
  }
...
tinyAPI.emitAsync = async function (event) {
  ...
  if (Array.isArray(tinyPlugins.order[event])) {
    for (const item in tinyPlugins.order[event]) {
      result = await tinyPlugins.order[event][item].callback.apply(
        {},
        argumentsFix(arguments, result),
      );
      if (!objType(result, 'object')) result = null;
