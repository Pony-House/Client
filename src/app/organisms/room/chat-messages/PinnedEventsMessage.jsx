...
export function comparePinEvents(content, prevContent) {
  // Preparing Validator
  const oldContent =
    objType(prevContent, 'object') && Array.isArray(prevContent.pinned)
      ? prevContent
      : { pinned: [] };
...
export default function PinnedEventsMessage({ user, comparedPinMessages, room }) {
  ...
  return added.length > 0 ? (
    <>
      ...
      {` pinned ${added.length === 1 ? 'a' : added.length} ${added.length === 1 ? 'message' : 'messages'} to this channel. See all `}
      <a href="#" onClick={openPinMessages}>
        pinned messages
      </a>
      .
    </>
    ...
    <>
      <strong>{twemojifyReact(user)}</strong>
      {` unpinned ${removed.length === 1 ? 'a' : removed.length} ${removed.length === 1 ? 'message' : 'messages'} to this channel. See all `}
      <a href="#" onClick={openPinMessages}>
        pinned messages
      </a>
      .
