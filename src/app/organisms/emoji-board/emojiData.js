...
const tinyBoardData = {
    emoji: { recent: [], fav: [], data: [], }, sticker: { recent: [], fav: [], data: [], }
};

export function getEmojiData(type, recent, fav, data, setRecent, setFav, setData) {
    if (objectHash(tinyBoardData[type].recent) !== objectHash(recent)) setRecent(tinyBoardData[type].recent);
...
export function loadEmojiData(selectedRoomId) {
    ...
    if (!selectedRoomId) {
        ...
        const emojiPacks = getRelevantPacks(mx).filter(
        ...
        const stickerPacks = getRelevantPacks(mx).filter(
